...
{
	init();
	Lcd_wr_commend(0x80+0x10);//set data address on first row  end 
	for(num=0;num<11;num++)
	{
		Lcd_wr_data(table1[num]);
		Delay(5);
	}
	Lcd_wr_commend(0x80+0x50);//+0x40+0x10,second row end
	for(num=0;num<8;num++)
	{
	...
	{
		//0x07 command output sample
		Lcd_wr_commend(0x18);//command:all monitor and mouse move-left
		Delay(200);
	}
	while(1);//wait
}